# WSGI application to automaticallly deploy biglm.biglm.net/2022-Fall/username
# inspired by:
...
BASEDIR = '/srv/biglm/www'
BASESITE = 'biglm.net'
SECRETS = [b'Bottou'] # Rotate this secret for each semester
SITES = ['biglm'] # Sites where this is allowed

def process_request(request, sitename='biglm', event=None):
    ...
    website_dir = f'{BASEDIR}/{sitename}'
    if BASESITE.startswith(sitename + '.'):
        website_url = f'https://{BASESITE}/'
    else:
        website_url = f'https://{sitename}.{BASESITE}/'
    try:
...
def application(environ, start_response):
    """WSGI application to update biglm.net/2022-Fall/username"""
    ...
    try:
        ...
        sitename = environ.get('HTTP_HOST', 'biglm').split('.')[0]
